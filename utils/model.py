...
class Model(nn.Module):
    def __init__(
        ...
        self.prompt_learner_neg = PromptLearner()  # 负分支专用
    ...
    def initialize_separate_prompt_learners(self):
        """初始化负分支PromptLearner为正分支的副本，促进热启动"""
        with torch.no_grad():
            # 复制正分支参数到负分支
            for neg_param, pos_param in zip(self.prompt_learner_neg.parameters(),
                                           self.prompt_learner_pos.parameters()):
                neg_param.data.copy_(pos_param.data)

            # 添加小噪声促进差异化学习
            for param in self.prompt_learner_neg.parameters():
                if param.requires_grad:
                    noise = torch.randn_like(param) * 0.01  # 1%的标准差噪声
                    param.add_(noise)
    ...
    def forward(self, visual, text, lengths, is_training=True, neg_text=None, return_text_features=False):
        """
        单/双分支前向:
            - 仅提供 text 时: 返回正分支 logits，用于训练/推理的原有流程
            - 同时提供 text 与 neg_text 且 is_training=True 时:
              返回 (logits_pos, logits_neg) 或 (logits_pos, logits_neg, text_features_pos, text_features_neg)
        """
        # 1. 获取视觉特征
        ...
        # 2. 正分支文本特征
        text_features_pos_raw = self.encode_text_prompt(text, visual_features)
        text_features_pos = text_features_pos_raw.unsqueeze(0).expand(
            visual_features.shape[0], text_features_pos_raw.shape[0], text_features_pos_raw.shape[1]
        )
        ...
        text_features_neg_raw = self.encode_text_prompt_neg(neg_text, visual_features)
        text_features_neg = text_features_neg_raw.unsqueeze(0).expand(
            visual_features.shape[0], text_features_neg_raw.shape[0], text_features_neg_raw.shape[1]
        )
        ...
        if return_text_features:
            return logits_pos, logits_neg, text_features_pos_raw, text_features_neg_raw
        else:
