...
class AdaptiveAdjacencyBuilder(nn.Module):
    ...
    def forward(self, delta, features=None, use_feature_sim=True):
        ...
        window_size = torch.round(window_size)  # [B, T] 保持float，避免CPU传输
        
        # === 向量化构建邻接矩阵（替代双重循环）===
        # 创建时间距离矩阵
        time_idx = torch.arange(T, device=device, dtype=torch.float32)  # [T]
        time_diff = torch.abs(time_idx.unsqueeze(0) - time_idx.unsqueeze(1))  # [T, T]
        
        # 广播比较：[B, T, 1] vs [1, T, T] -> [B, T, T]
        window_size_expanded = window_size.unsqueeze(2)  # [B, T, 1]
        time_diff_expanded = time_diff.unsqueeze(0)      # [1, T, T]
        
        # 向量化判断：时间距离 <= 窗口大小
        # 对每个batch的每个时间步t，判断与其他时间步的距离是否在窗口内
        adj = (time_diff_expanded <= window_size_expanded).float()  # [B, T, T]
        ...
        # 添加自连接
        identity = torch.eye(T, device=device, dtype=torch.float32).unsqueeze(0)  # [1, T, T]
        adj = adj + identity
