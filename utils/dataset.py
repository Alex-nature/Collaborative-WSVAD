...
class UCFDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        feature_path = os.path.normpath(original_path.replace('/data', self.base_path))
...
def make_ucf_dataloader(split_mode: str, clients_num: int, batch_size: int, visual_length: int):
    ...
    for i in range(clients_num):
        # 使用os.path.join构建路径
        train_list = os.path.join('.', 'data', 'list', f'ucf_{split_mode}', f'ucf_{split_list[i]}.csv')
    ...
    # 使用os.path.join构建路径
    test_list = os.path.join('.', 'data', 'list', 'ucf_test.csv')
...
def make_xd_dataloader(split_mode: str, clients_num: int, batch_size: int, visual_length: int):
    ...
    for i in range(clients_num):
        # 使用os.path.join构建路径
        train_list = os.path.join('.', 'data', 'list', f'xd_{split_mode}', f'xd_{split_list[i]}.csv')
        train_dataset = XDDataset(visual_length, train_list, False)
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
        train_loaders.append(train_loader)

    # 使用os.path.join构建路径
    test_list = os.path.join('.', 'data', 'list', 'xd_test.csv')
