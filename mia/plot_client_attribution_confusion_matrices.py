...
from mia.train_client_attribution_model import (
    GROUP_COLUMN,
    LABEL_COLUMN,
    MEMBERSHIP_COLUMN,
    group_split,
...
def plot_confusion(y_true, y_pred, labels, title: str, output_path: Path, normalize: str | None):
    ...
    disp.plot(ax=ax, cmap="Blues", colorbar=True, values_format=value_format, xticks_rotation=45)
    ax.set_title(title)
    fig.tight_layout()
...
def main():
    ...
    model_specs = {
        "logistic_regression": models_dir / f"{args.name}_logistic_regression_client_attribution.pkl",
        "random_forest": models_dir / f"{args.name}_random_forest_client_attribution.pkl",
    }
    ...
    for model_name, model_path in model_specs.items():
        ...
        output_path = results_dir / f"{args.name}_{model_name}_client_attribution_confusion_matrix_{suffix}.png"
        plot_confusion(
            y_test,
            y_pred,
            labels=labels,
